...
export async function startCommand() {
  ...
  async function addDomains(domains) {
    const spinner = ora('🧩 Waiting for admin approval...').start();

    try {
      let cmd, args;
      if (isWindows) {
        cmd = getHostPilotPath();
        args = ['addHost', ...domains];
      } else {
        cmd = 'sudo';
        args = [getHostPilotPath(), 'addHost', ...domains];
      }
      ...
      console.log(chalk.green('✅ Domains added successfully.'));
      return res;
    } catch (err) {
      spinner.stop();
  ...
  for (const app of config.apps) {
    const customPath = getCustomAppConfPath(
      process.cwd(),
      app.customAppConfig
    );

    const confContent = await buildNginxConfig({
      domain: app.domain,
      port: app.port,
      https: app.https === true,
      customAppConfigPath: customPath,
    });
